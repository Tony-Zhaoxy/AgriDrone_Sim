...
def generate_launch_description():
    ...
    args = [
        DeclareLaunchArgument(
            'vio_backend', default_value='openvins',
            description='VIO backend: openvins | orbslam3'),

        DeclareLaunchArgument(
            'orb_vocab', default_value='/opt/ORB_SLAM3/Vocabulary/ORBvoc.txt',
            description='Path to ORBvoc.txt (only used when vio_backend=orbslam3)'),

        DeclareLaunchArgument(
            'orb_pkg', default_value='orb_slam3_ros2',
            description='ORB_SLAM3 ROS 2 wrapper package name'),

        DeclareLaunchArgument(
            'orb_exe', default_value='stereo_inertial',
        ...
        DeclareLaunchArgument(
            'require_marker_init', default_value='true',
    ...
    # RealSense include: use the wrapper launch that already works from CLI
